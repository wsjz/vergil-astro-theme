...
export function processMediaDirective(node) {
    ...
    if (name === 'video') {
        ...
        if (safeCssValue(width)) containerStyle += `--video-width:${safeCssValue(width)};`;
        if (safeCssValue(align)) containerStyle += `--video-align:${safeCssValue(align)};`;
        ...
        if (src) {
            ...
            const ytId = /^[\w-]{6,20}$/.test(youtube) ? youtube : '';
            let iframeSrc = `https://www.youtube.com/embed/${ytId}?rel=0`;
        ...
        let containerStyle = '';
        if (safeCssValue(align)) containerStyle += `--audio-align:${safeCssValue(align)};`;
        if (safeCssValue(width)) containerStyle += `--audio-width:${safeCssValue(width)};`;
        ...
        if (netease) {
            ...
            node.children = [{
                type: 'html',
                value: `<div class="md-audio-netease-wrap${isCard ? ' md-audio-netease-wrap-card' : ''}"><iframe src="//music.163.com/outchain/player?type=2&id=${encodeURIComponent(netease)}&auto=0&height=${encodeURIComponent(playerHeight)}" frameborder="no" border="0" marginwidth="0" marginheight="0" width="${iframeWidth}" height="${iframeHeight}" title="${escapeHtml(title || 'Netease Music')}"></iframe></div>`
            }];
        } else if (voice) {
            node.data = { hName: 'div', hProperties: { class: 'md-directive md-directive-audio md-audio-voice', 'data-src': safeUrl(voice), 'data-duration': duration, ...(containerStyle ? { style: containerStyle } : {}) } };
            node.children = [{
                type: 'html',
