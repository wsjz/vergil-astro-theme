...
这一点容易被忽略：**相册里的拍摄参数不是一直显示的**。

在桌面端，参数藏在照片的悬停浮层里，鼠标移到照片上才会出现。在触屏设备上没有悬停这个动作，所以参数会常驻显示。两套主题显示的字段不完全一样：

| 主题 | 显示的字段 |
|------|-----------|
| Golden | 品牌、机型、镜头、焦距、光圈、快门、ISO |
| Seasons | 品牌、镜头、焦距、光圈 |

想直接看效果，去演示站的 [四季物语](/albums/album-2/) 把鼠标移到任意一张照片上。

### 在文章里单独展示一张照片

如果只是想在文章中间插一张带参数的照片，不必建相册，用 `photo` 指令就够了。它会生成模糊背景加居中照片的版式，底部带一条品牌与参数信息栏。

:::photo{src="https://images.unsplash.com/photo-1506905925346-21bda4d32df4?w=1200&q=80" alt="山间湖泊" brand="fujifilm" model="X-T5" focal="35mm" aperture="f/1.4" shutter="1/125s" iso="800" datetime="2026-04-12 17:40"}
:::

写法：

```markdown
:::photo{src="./photo.jpg" alt="山间湖泊" brand="fujifilm" model="X-T5"
  focal="35mm" aperture="f/1.4" shutter="1/125s" iso="800" datetime="2026-04-12 17:40"}
:::
```
...
| `src` | 图片地址，必填 |
| `alt` | 替代文字 |
| `brand` | 相机品牌，内置二十个厂商的标志，写小写英文名即可，例如 `fujifilm`、`canon`、`nikon`、`sony`、`leica`、`apple` |
| `logo` | 单独指定用哪个品牌的标志，不填则跟随 `brand` |
| `model` | 机型 |
| `focal` / `aperture` / `shutter` / `iso` | 焦距、光圈、快门、感光度，显示在信息栏右侧 |
| `datetime` | 拍摄时间 |
| `fancybox` | 设为 `false` 可关闭点击放大 |

品牌名不在内置列表里也没关系，`brand` 会原样显示成文字。
