...
|------|------|---------|
| `grid` | 规则网格排列 | 已实现 |
| `masonry` | 瀑布流，高度自适应 | **尚未实现**，当前渲染为 grid |
| `timeline` | 按时间线排列 | **尚未实现**，当前渲染为 grid |
| `carousel` | 轮播图展示 | **尚未实现**，当前渲染为 grid |

:::callout{type="warn" title="目前只有 grid 有实际效果"}
schema 接受这四个值，但样式层只实现了 `grid`。填另外三个值不会报错，`data-layout` 属性会正确输出，只是视觉上和 `grid` 没有区别。
:::
