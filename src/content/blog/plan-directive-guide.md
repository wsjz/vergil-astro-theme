...
基于艾宾浩斯遗忘曲线自动生成复习计划。输入学习日期，自动计算 6 个复习节点（+1天、+2天、+4天、+7天、+15天、+30天）。鼠标悬停节点可查看复习详情（自动显示表格中其他列的内容）。
...
- `dateCol="学习日期"` 指定学习日期列（必填）
- `titleCol="内容"` 指定标题列
- `steps="1,2,4,7,15,30"` 自定义复习间隔天数，默认 `1,2,4,7,15,30`
- 节点状态：实心 = 已完成，呼吸动画 = 今天该复习，空心 = 待复习
- **鼠标悬停节点自动显示表格中其他列的内容**（如"复习重点"、"预计时长"）
- 右侧百分比表示已完成复习节点占比
- 和 `table`、`board` 视图组合，可切换查看原始数据或看板分组

**自定义复习间隔示例：**

```markdown
:::plan{views="ebbinghaus" dateCol="学习日期" steps="1,3,7,14,30,60,90"}
| 内容:text | 学习日期:date |
|-----------|---------------|
| 单词 List 1 | 2026-05-20    |
:::
```

- `steps="1,3,7,14,30,60,90"` 将复习节点扩展到 90 天，共 7 个节点
- 间隔用逗号分隔，必须是正整数
- 不指定时默认使用艾宾浩斯标准间隔：1, 2, 4, 7, 15, 30 天
...
|-------------|-----------|-----------|---------------|-------------|
| 科幻        | 三体      | 刘慈欣    | 2024-02-15    | 9.0           |
| 文学        | 活着      | 余华      | 2024-03-10    | 9.1          |
| 科幻        | 沙丘      | 赫伯特    | 2024-05-20    | 8.5           |
| 历史        | 万历十五年 | 黄仁宇   |               |             |
| 文学        | 百年孤独  | 马尔克斯  |               |             |
:::
...
|-----------|-----------|-------------|-----------|
| 抵达昆明  | 2024-07-01 14:59 | done        | 入住翠湖附近 |
